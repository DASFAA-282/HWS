...
void sift_test1B(
  ...
  int topk
	...
	elem* norm_sqr = new elem[vecsize];
...
/*	
	for (int i = 0; i < vecsize; i++){
		for (int j = 0; j < M; j++){
			for(int l = 0; l < d; l++)
				data[norm[i].id][j * d + l] = vec[j][dip[norm[i].id][j]][l];
		}
	}
*/	
	for (int i = 0; i < vecsize; i++){
		for (int j = 0; j < D; j++){
			data[norm[i].id][j] = data[norm[i].id][j] / norm_sqr[i].ip;		
		}
	}
    ...
    cout << "Building index:\n";
     appr_alg = new HierarchicalNSW<float>(&l2space, vecsize, r_num, num_ring, M2, efConstruction);
        ...
        appr_alg->addPoint((void *) (dip[ norm[0].id ]), (size_t) (norm[0].id), Layer[0]-1, norm[0].ip,  dist_book, data[norm[0].id], data);
...
#pragma omp critical
			...
			if(Layer[j2] >= 1){
                appr_alg->addPoint((void *) (dip[ norm[j2].id ]), (size_t) (norm[j2].id), Layer[j2]-1, norm[j2].ip, dist_book, data[norm[j2].id], data);
            }                   
        }
		
		float* zero_arr = new float[D];
		for(int i = 0; i < D; i++)
			zero_arr[i] = 0;
		
		appr_alg->addPoint((void *) (dip[ norm[0].id ]), (size_t) (norm[0].id), -1, norm[0].ip, dist_book, zero_arr, data);
