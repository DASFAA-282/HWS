...
*/
        ...
        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
        searchMobiusLayer(tableint ep_id, int level, float* query_data, float** data_org) {
            VisitedList *vl = visited_list_pool_->getFreeVisitedList();
            vl_type *visited_array = vl->mass;
            vl_type visited_array_tag = vl->curV;
            ...
            */
            ...
            //dist = 	fstl2func2_((unsigned char*) data_point, (unsigned char*)getQuanByInternalId(ep_id, level), dist_book, quan_size_);		
			dist = fstl2func2_(query_data, data_org[trans_quan[ep_id]], dist_func_param_);
            ...
            while (!candidateSet.empty()) {
                ...
                unsigned int *data;// = (int *)(linkList0_ + curNodeNum * size_links_per_element0_);				
				data = getLinkByInternalId2(curNodeNum, level);
...
//                    if (candidate_id == 0) continue;
					...
					*/
					dist1 = fstl2func2_(query_data, data_org[trans_quan[candidate_id]], dist_func_param_);
    ...
    void getNeighborsByHeuristic3(
	        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> &topResults,
 			const size_t NN, int level, float** data_org)
    {
        ...
        for (std::pair< dist_t, tableint> curen2 : returnlist) {
          ...
          dist_t curdist =
            curnorm * fstl2func_((unsigned char*)getQuanByInternalId2(curen2.second, level), (unsigned char*)getQuanByInternalId2(curen.second, level), 
			        dist_book, quan_size_);;
			*/		
			dist_t curdist = fstl2func2_(data_org[trans_quan[curen2.second]], data_org[trans_quan[curen.second]], dist_func_param_);			
          if (curdist < dist_to_query) {
            good = false;
            break;
          }
      ...
      for (std::pair< dist_t, tableint> curen2 : returnlist) {
        ...
        void mutuallyConnectNewElement(const void *data_point, tableint cur_c,
                                       ...
                                       int level, float*** dist_book, labeltype label, float** data_org) {
			...
			getNeighborsByHeuristic3(ip_candidates, M_, level, data_org);
            ...
            for (size_t idx = 0; idx < rez.size(); idx++) {
                ...
                if (sz_link_list_other < Mcurmax) {
                    ...
                    dist_t d_max = norm * fstl2func_((unsigned char*)getQuanByInternalId2(cur_c, level), (unsigned char*)getQuanByInternalId2(rez[idx], level), 
					        dist_book, quan_size_);
					*/		
					dist_t d_max = 	fstl2func2_(data_org[trans_quan[cur_c]], data_org[trans_quan[rez[idx]]], dist_func_param_);	
                    ...
                    for (size_t j = 0; j < sz_link_list_other; j++) {
						...
						*/
                        candidates.emplace(
                                fstl2func2_(data_org[trans_quan[data[j]]], data_org[trans_quan[rez[idx]]], dist_func_param_), data[j]);						
                    }

                    //getNeighborsByHeuristic3(candidates, Mcurmax, level, dist_book);
                    getNeighborsByMobius(candidates, Mcurmax, level, data_org);
        ...
        void mutuallyConnectMobius(const void *query_data, tableint cur_c,
									   std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> ip_candidates,
                                       int level, float** data_org) {

            size_t Mcurmax = maxM_;
	
			getNeighborsByMobius(ip_candidates, M_, level, data_org);
			
            if (ip_candidates.size() > M_)
                throw std::runtime_error("Should be not be more than M_ candidates returned by the heuristic");
            /*
            std::vector<tableint> selectedNeighbors;
            selectedNeighbors.reserve(M_);
			...
			*/
...

            {
				...
				*/
                linklistsizeint* ll_cur = getLinkByInternalId2(cur_c, level);
                if (*ll_cur) {
                    throw std::runtime_error("The newly inserted element should have blank link list");
                }
 			...
 			const size_t NN, int level, float** data_org)
    {
      ...
      for (std::pair< dist_t, tableint> curen2 : returnlist) {
        ...
        void addPoint(const void *quan_point, labeltype label, int layer, float norm, float*** dist_book, float* query, float** data_org) {
            addPoint2(quan_point, label, layer, norm, dist_book, query, data_org);
        }

        tableint addPoint2(const void *quan_point, labeltype label, int layer, float norm, float*** dist_book, float* query_data, float** data_org) {
            ...
            if(layer < 0){
                printf("check1\n");
                std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> ip_candidates = searchMobiusLayer(
                    currObj1, maxlevel_, query_data, data_org);	
                printf("check2\n");
                 mutuallyConnectMobius(query_data, ring_elements_[maxlevel_]-1, ip_candidates, maxlevel_, data_org);
                 printf("check3\n");					
			}
            ...
            if ((signed)currObj0 != -1) {
                ...
                if (curlevel < maxlevelcopy) { // only for outest ring 
					...
					curdist = fstl2func2_(query_data, data_org[trans_quan[currObj1]], dist_func_param_);
                    for (int level = maxlevelcopy; level > curlevel; level--) {
                        ...
                        while (changed) {
                            ...
                            for (int i = 0; i < size; i++) {
                                ...
                                dist_t d = fstl2func2_(query_data, data_org[trans_quan[cand]], dist_func_param_);
                                if (d < curdist) {
                                    curdist = d;
                                    currObj1 = cand;
                                    changed = true;
                ...
                for (int level = std::min(curlevel, maxlevelcopy); level >= 0; level--) {
                    ...
                    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> ip_candidates = searchMobiusLayer(
                            currObj1, level, query_data, data_org);

                    mutuallyConnectNewElement(quan_point, cur_c, top_candidates, ip_candidates, level, dist_book, label, data_org);
        ...
        searchKnn(const void *query_data, size_t K, float** quan_book, float* max_norm, int vecdim,  double* avg_dist) {
			...
			//unsigned *neighbors = (unsigned *)(getLinkByInternalId(ring_elements_[maxlevel_], maxlevel_));
			unsigned *neighbors = (unsigned *)(getLinkByInternalId2(ring_elements_[maxlevel_]-1, maxlevel_));
			unsigned MaxM = *neighbors;
			neighbors++;
			
			for (unsigned i = 0; i < 1; i++) {
				unsigned iid = neighbors[0];
                float *x = (float *)(getNormByInternalId2(iid, maxlevel_));
                float norm_x = *x;
                x++;
                float dist = quan_dist((unsigned char*) x, quan_book, 0);
				retset2[i] = Neighbor(iid, norm_x * dist, true);             
				visited_array[iid] = visited_array_tag;
				L_ip++;
            }
            ...
            for (unsigned m = 1; m < MaxM; ++m) {
                ...
                if (visited_array[id] == visited_array_tag) continue;
                    ...
                    float *data = getNormByInternalId2(neighbors[m], maxlevel_);
                    float norm = *data;
                    data++;
            ...
            int k;			
			for (int level = maxlevel_; level >= maxlevel_; level--) {			
