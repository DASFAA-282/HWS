...
namespace hnswlib {
  ...
  public:
    ...
    std::priority_queue< std::pair< dist_t, labeltype >> searchKnn(void *query_data, int k, double* avg_dist) {
      tableint currObj = enterpoint_node;
    ...
    void       LoadIndex(const string &location, SpaceInterface<dist_t> *s)
    {
      ...
      printf("size_data_per_element_ = %d\n", size_data_per_element_);
   ...
   void adjustnorm(float* mass, size_t cur_id, float* norm, float* thres, double* coff, size_t vecdim_) {
	   /*
      tableint currObj = enterpoint_node;
      dist_t curdist = fstdistfunc_(query_data, getDataByInternalId(enterpoint_node), dist_func_param_);
      dist_calc++;
      ...
      */
      float* query_data = (float *) getDataByInternalId(cur_id);
	  tableint currObj = cur_id;
      std::priority_queue< std::pair< dist_t, tableint  >, vector<pair<dist_t, tableint>>, CompareByFirst> topResults = searchBaseLayerST(currObj, query_data, ef2_);
      //----------------NOPP---------------------------------
	  
	   
	  int ext_id = getExternalLabel(cur_id);
	  float alpha_;
	  if(norm[ext_id] < thres[N1 -2])
		  alpha_ = coff[N1-1];
	  else{
	     for(int i= 0; i < N1-1; i++){
		    if(norm[ext_id] >= thres[i]){
			  alpha_ = coff[i];
			  break;
	  	    }
	     }
	  ...
	  int x = topResults.size();
	  float* arr0 = new float[x];
	  int* arr1 = new int[x]; 
	  
	   int y = x-1;
	   while (topResults.size() > 0) {
          arr0[y] = topResults.top().first;
          arr1[y] = topResults.top().second;
		  y--;
		  topResults.pop();
      }
	  ...
	  printf("cur-id = %d --- x = %d --- maxM = %d\n", cur_id, x, maxM0_);
	  
	  printf("[1] = %d; [2] = %d; [3] = %d\n", arr1[0], arr1[1], arr1[2]);
	  
	  printf("ip = [1] = %f; [2] = %f; [3] = %f\n", arr0[0], arr0[1], arr0[2]);
	  
	  float yyy = 0;
	  int ext_id2 = getExternalLabel(arr1[0]);
	  for(int i = 0; i < vecdim_; i++){
		  yyy += mass[(size_t)ext_id * vecdim_ + i] * mass[(size_t)ext_id2 * vecdim_ + i];
	  }
	  printf("yyy = %f\n", yyy);
	  ...
	  */
	  ...
	  int* final_id = new int[maxM0_];
	  for(int i = 0; i < x; i++){
		  if(cur_id == arr1[i]) continue;  //cur_id = internal id
		  fflag = true;
		  
		  int id2 = getExternalLabel(arr1[i]);
		  for(int j = 0; j < pos; j++){
			  int id3 = getExternalLabel(final_id[j]);
			  
			  
			  float u = compare00(mass + id2 * vecdim_, mass + id3 * vecdim_, (unsigned) vecdim_ );
			  
			  if(alpha_ * (-1) * arr0[i] <  u){
				  fflag = false;
				  break;
			  }
		  }
	  ...
	  linklistsizeint *ll_cur = (linklistsizeint *)(data_level0_memory_ + cur_id * size_data_per_element_ + offsetLevel0_);
	  *ll_cur = pos;
	  
	  tableint *data = (tableint *)(ll_cur + 1);
      for (int idx = 0; idx < pos; idx++) {
	      data[idx] = final_id[idx];
	  }
