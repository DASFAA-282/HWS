...
int loadFvecs(float*& data, int numItems, std::string inputPath, int dimension) {
...
int main(int argc, char** argv) {
    ...
    if (mode == "database") {
        ...
        float *mass = NULL;
        size_t datadim = loadFvecs(mass, vecsize, dataname, vecdim);
...
#pragma omp parallel for
        for (size_t i = 1; i < vecsize; i++) {
            appr_alg->addPoint((void *)(mass + i*vecdim), (size_t)i);
        }
		...
		std::vector<std::priority_queue<std::pair<float, int >>> answers;
		std::priority_queue<std::pair<float, int>> queue_;
		
             
		std::pair<float, int> pair_;
                int MAXK = 100;
                int* massb = new int[MAXK];
                std::ifstream inputGT("truth.gt", std::ios::binary);
                for(int i = 0; i < qsize; i++){
                    int in = 0;
                    inputGT.read((char *) &in, 4);
                    inputGT.read((char *) massb, 4 * MAXK);
                     for(int j = 0; j < 100; j ++){   //qsize
                      
                        pair_.first = 50-j;
                        pair_.second = massb[j];
                        queue_.push(pair_);
                    }
                    answers.push_back(queue_); 
        ...
        hnswlib::L2Space l2space(vecdim);
        float *massQ = NULL;
        size_t datadim = loadFvecs(massQ, qsize, queryname, vecdim);
        ...
        int *temp_data1 = NULL;
        int *temp_data2 = NULL;
        size_t degree_count_ip = 0;
        size_t degree_count_cos = 0;
        ...
        std::cout << "avg. ip degree = " << (double)degree_count_ip / appr_alg->maxelements_  << std::endl;
        std::cout << "avg. cos degree = " << (double)degree_count_cos / appr_alg->maxelements_  << std::endl;
       ...
       // topK = 1;
        for (int efSearch : efs) {
            ...
            for (int i = 0; i < qsize; i++) { //qsize
                std::vector <int> res;
                while (!gt[i].empty()) {
                    res.push_back(gt[i].top().second);
                   // printf("i = %d; id = %d ip = %.5f\n", i, gt[i].top().second, gt[i].top().first);
                    gt[i].pop();
                }
                std::reverse(res.begin(), res.end());

                std::unordered_set<labeltype> g;
                std::priority_queue<std::pair<float, int >> gt(answers[i]);
                total += gt.size();
		//	printf("i = %d---total = %d\n",i,total);
                while (gt.size()) {
